...
let rec typchk_expr (tc:typctx) (e:iexpr) : exprtyp = 
  ...
  (match e with 
    ...
    | Abstraction (ali,c,li) -> (match (typchk_cmd (List.fold_left (fun b (variable,itype,li) -> update b variable (VTyp(itype,true))) tc ali) c) with TypCtx tc1 -> (match tc1 "ret" with VTyp (t,true) -> ExpTyp (TypFunc ((List.fold_left (fun b (variable,itype,li) -> itype::b) [] ali),t)) | VTyp(_,false) -> eerr li ("variable ret used uninitialized") | Undeclared -> eerr li ("variable ret used undeclared")) | CTypErr s -> eerr li s)
    | Apply (e1,eli,li) -> match (typchk_expr tc e1) with 
        ExpTyp TypFunc (x,y) -> if x= (List.fold_left (fun b h-> match h with ExpTyp x -> x::b) [] (List.map (typchk_expr tc) eli)) 
        ...
        | ETypErr s -> eerr li ("invalid function definition")
